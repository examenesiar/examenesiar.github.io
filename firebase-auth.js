...
function mostrarLogin(mensajeError) {
  ...
  if (passField && passField.type === "text") {
    ...
    if (toggleBtn) { toggleBtn.textContent = "👁"; toggleBtn.style.color = "#94a3b8"; }
  }
...
function mostrarBarraSesion(email, licencia) {
  ...
  btnLogout.addEventListener("click", () => window._confirmarCierreSesion());
...
window._confirmarCierreSesion = function() {
  ...
  try {
    ...
    if (_rawSt) {
      var _simSt = JSON.parse(_rawSt)['simulacro_iar'];
      if (_simSt && !_simSt.totalShown) {
        var _nG = _simSt.graded ? Object.keys(_simSt.graded).filter(function(k){ return _simSt.graded[k]; }).length : 0;
        var _nA = _simSt.answers ? Object.keys(_simSt.answers).filter(function(k){ var a = _simSt.answers[k]; return Array.isArray(a) && a.length > 0; }).length : 0;
        if (_nG > 0 || _nA > 0) _haySimulacroEnCurso = true;
      }
    }
  } catch(e) {}
  ...
  if (!document.getElementById("logout-confirm-styles")) {
    ...
    s.textContent = `
      ...
      .logout-confirm-icon { font-size:2.5rem;margin-bottom:10px; }
      .logout-confirm-titulo { font-size:1.1rem;font-weight:800;color:#1e3a8a;margin-bottom:8px; }
      .logout-confirm-msg { font-size:.9rem;color:#475569;line-height:1.6;margin-bottom:20px; }
      .logout-confirm-advertencia {
        background:#fef3c7;border:1.5px solid #f59e0b;border-radius:8px;
        padding:10px 12px;margin-bottom:16px;font-size:.85rem;
        color:#92400e;line-height:1.5;text-align:left;
      }
      .logout-confirm-btns { display:flex;gap:10px; }
      .logout-confirm-cancelar {
        flex:1;padding:11px;background:#f1f5f9;color:#475569;
        border:1.5px solid #cbd5e1;border-radius:8px;
        font-size:.92rem;font-weight:700;cursor:pointer;
      }
      .logout-confirm-aceptar {
        flex:1;padding:11px;
  ...
  var _advertenciaHTML = _haySimulacroEnCurso
    ? `<div class="logout-confirm-advertencia">⚠️ <strong>Tenés un Simulacro IAR en curso.</strong><br>Al cerrar sesión, el progreso de las respuestas marcadas se perderá y la próxima vez se generará un nuevo set de 20 preguntas.</div>`
    : '';
  overlay.innerHTML = `
    <div class="logout-confirm-box">
      <div class="logout-confirm-icon">🔐</div>
      <div class="logout-confirm-titulo">¿Cerrar sesión?</div>
      <div class="logout-confirm-msg">
        Tu progreso guardado se conservará en la nube.<br>
        Podés continuar desde cualquier dispositivo.
      </div>
      ${_advertenciaHTML}
      <div class="logout-confirm-btns">
        <button class="logout-confirm-cancelar" id="logout-confirm-cancelar-btn">Cancelar</button>
        <button class="logout-confirm-aceptar" id="logout-confirm-aceptar-btn">Cerrar sesión</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);
  document.getElementById("logout-confirm-cancelar-btn").addEventListener("click", () => overlay.remove());
  document.getElementById("logout-confirm-aceptar-btn").addEventListener("click", () => {
    overlay.remove();
    handleLogout();
  });
};
...
function _limpiarTimerSimulacroIAR() {
  ...
  if (window._simulacroTimer && typeof window._simulacroTimer.limpiar === 'function') {
    try { window._simulacroTimer.limpiar(); } catch(e) {}
  }
}
...
let _preguntasEditor = null;
let _cambiosPendientes = {};

function _inyectarEstilosModificar() {
  if (document.getElementById('mod-resp-styles')) return;
  const s = document.createElement('style');
  s.id = 'mod-resp-styles';
  s.textContent = `
    .mod-pregunta-card {
      background:#fff;border-radius:12px;padding:20px 22px;margin-bottom:18px;
      border:1.5px solid #e2e8f0;box-shadow:0 2px 8px rgba(0,0,0,.06);position:relative;
    }
    .mod-pregunta-card.tiene-cambios { border-color:#f59e0b; }
    .mod-pregunta-num { font-size:.72rem;font-weight:800;color:#64748b;text-transform:uppercase;letter-spacing:.06em;margin-bottom:8px; }
    .mod-enunciado-texto { font-size:.97rem;color:#1e293b;line-height:1.6;margin-bottom:14px; }
    .mod-textarea {
      width:100%;border:1.5px solid #f59e0b;border-radius:8px;
      padding:10px 12px;font-size:.93rem;color:#1e293b;line-height:1.6;
      resize:vertical;min-height:80px;box-sizing:border-box;
      -webkit-user-select:text!important;user-select:text!important;font-family:inherit;
    }
    .mod-btn-reescribir {
      background:#f1f5f9;color:#475569;border:1.5px solid #cbd5e1;
      border-radius:6px;padding:4px 12px;font-size:.8rem;font-weight:700;
      cursor:pointer;transition:all .15s;white-space:nowrap;
    }
    .mod-btn-reescribir:hover { background:#e2e8f0; }
    .mod-opcion-row { display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid #f1f5f9;flex-wrap:wrap; }
    .mod-opcion-row:last-child { border-bottom:none; }
    .mod-opcion-letra { font-weight:800;font-size:.9rem;color:#475569;min-width:20px; }
    .mod-opcion-texto { flex:1;font-size:.9rem;color:#1e293b;line-height:1.45;word-break:break-word;min-width:0; }
    .mod-opcion-input {
      flex:1;border:1.5px solid #f59e0b;border-radius:6px;padding:6px 10px;font-size:.88rem;color:#1e293b;
      -webkit-user-select:text!important;user-select:text!important;font-family:inherit;min-width:0;box-sizing:border-box;
    }
    .mod-select-correcta { padding:4px 8px;border-radius:6px;font-size:.78rem;font-weight:700;border:1.5px solid #cbd5e1;cursor:pointer; }
    .mod-select-correcta.correcta { background:#d1fae5;color:#065f46;border-color:#6ee7b7; }
    .mod-select-correcta.incorrecta { background:#fee2e2;color:#dc2626;border-color:#fca5a5; }
    .mod-btn-guardar {
      background:linear-gradient(135deg,#0d7490,#0891b2);color:#fff;border:none;border-radius:8px;
      padding:9px 22px;font-size:.88rem;font-weight:700;cursor:pointer;transition:all .15s;margin-top:14px;
    }
    .mod-btn-guardar:hover { transform:translateY(-1px);box-shadow:0 4px 12px rgba(13,116,144,.25); }
    .mod-btn-descartar {
      background:#f1f5f9;color:#475569;border:1.5px solid #cbd5e1;border-radius:8px;
      padding:9px 16px;font-size:.85rem;font-weight:700;cursor:pointer;transition:all .15s;margin-top:14px;margin-left:8px;
    }
    .mod-btn-descartar:hover { background:#e2e8f0; }
    #mod-pass-overlay {
      position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.65);z-index:999990;
      display:flex;justify-content:center;align-items:center;
      font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;
    }
    .mod-pass-box { background:#fff;border-radius:16px;padding:36px 32px;width:100%;max-width:380px;box-shadow:0 20px 60px rgba(0,0,0,.3);text-align:center;margin:16px; }
    #mod-confirm-overlay {
      position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.6);z-index:999995;
      display:flex;justify-content:center;align-items:center;
      font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;
    }
...
async function _mostrarModalContrasena(callback) {
  ...
  try {
    const snap = await getDoc(doc(db, 'config', 'admin_secret'));
    if (snap.exists()) {
      passwordCorrecta = snap.data().password || null;
    }
  } catch(err) {
    console.error('[IAR Mod14] Error al obtener contraseña:', err);
    alert('⚠️ No se pudo obtener la contraseña. Error: ' + (err.message || err.code || err));
    callback(false);
    return;
  }

  if (!passwordCorrecta) {
    alert('⚠️ No se encontró la contraseña en Firestore.\nVerificá: colección "config" → documento "admin_secret" → campo "password".');
    callback(false);
    return;
  }
  ...
  overlay.innerHTML = `
    <div class="mod-pass-box">
      ...
      <div style="font-size:1.15rem;font-weight:800;color:#0d7490;margin-bottom:6px;">Acceso Restringido</div>
      <div style="font-size:.87rem;color:#64748b;margin-bottom:20px;line-height:1.5;">
        Para modificar las preguntas ingresá la contraseña de administrador.
      </div>
      <div id="mod-pass-error" style="display:none;background:#fee2e2;color:#dc2626;border:1px solid #fca5a5;border-radius:8px;padding:8px 12px;font-size:.83rem;font-weight:600;margin-bottom:12px;"></div>
      <div style="text-align:left;margin-bottom:14px;">
        <label style="display:block;font-size:.8rem;font-weight:700;color:#475569;margin-bottom:5px;text-transform:uppercase;letter-spacing:.04em;">Contraseña</label>
        <div style="position:relative;">
          <input id="mod-pass-input" type="password" placeholder="••••••••"
            style="width:100%;padding:11px 40px 11px 14px;border:1.5px solid #cbd5e1;border-radius:8px;font-size:.95rem;color:#1f2937;outline:none;box-sizing:border-box;-webkit-user-select:text!important;user-select:text!important;" />
          <button id="mod-pass-toggle" type="button" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;color:#94a3b8;font-size:1.1rem;padding:0;">👁</button>
        </div>
      </div>
      <div style="display:flex;gap:10px;">
        <button id="mod-pass-cancelar" style="flex:1;padding:11px;background:#f1f5f9;color:#475569;border:1.5px solid #cbd5e1;border-radius:8px;font-size:.92rem;font-weight:700;cursor:pointer;">Cancelar</button>
        <button id="mod-pass-confirmar" style="flex:1;padding:11px;background:linear-gradient(135deg,#0d7490,#0891b2);color:#fff;border:none;border-radius:8px;font-size:.92rem;font-weight:700;cursor:pointer;">Ingresar</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);

  const input = document.getElementById('mod-pass-input');
  const errDiv = document.getElementById('mod-pass-error');

  document.getElementById('mod-pass-toggle').addEventListener('click', () => {
  ...
  function intentar() {
    if (input.value === passwordCorrecta) {
      ...
      errDiv.textContent = 'Contraseña incorrecta. Intentá nuevamente.';
      errDiv.style.display = 'block';
      input.value = '';
      input.focus();
    }
  ...
  document.getElementById('mod-pass-confirmar').addEventListener('click', intentar);
  input.addEventListener('keydown', e => { if (e.key === 'Enter') intentar(); });
  document.getElementById('mod-pass-cancelar').addEventListener('click', () => {
    overlay.remove();
    callback(false);
  });
...
window.mostrarModificarRespuestas = function() {
  ...
  _mostrarModalContrasena(ok => {
    if (!ok) return;
    _modificarAutorizado = true;
    _abrirPanelModificar();
  });
};

function _abrirPanelModificar() {
  _inyectarEstilosModificar();
  document.querySelectorAll('.menu-principal, #cuestionario, #panel-respuestas-correctas, #pagina-respuestas-examen, #buscador-preguntas').forEach(el => {
    if (el) { el.style.display = 'none'; el.classList && el.classList.add('oculto'); }
  });
  const panel = document.getElementById('modificar-respuestas-panel');
  if (panel) panel.style.display = 'block';
  document.getElementById('modificar-submenu').style.display = '';
  document.getElementById('modificar-editor').style.display = 'none';
  _cambiosPendientes = {};
  window.scrollTo(0, 0);
}

window.cerrarModificarRespuestas = function() {
  const panel = document.getElementById('modificar-respuestas-panel');
  if (panel) panel.style.display = 'none';
  const menu = document.getElementById('menu-principal');
  if (menu) { menu.style.display = ''; menu.classList && menu.classList.remove('oculto'); }
  window.scrollTo(0, 0);
};

window.volverAlSubmenuModificar = function() {
  if (Object.keys(_cambiosPendientes).length > 0) {
    if (!confirm('Tenés cambios sin guardar. ¿Salir de todas formas?')) return;
  }
  _cambiosPendientes = {};
  _seccionEditorActual = null;
  _preguntasEditor = null;
...
window.abrirEditorExamen = async function(seccionId) {
  _seccionEditorActual = seccionId;
  _cambiosPendientes = {};
  document.getElementById('modificar-submenu').style.display = 'none';
  const editor = document.getElementById('modificar-editor');
  editor.style.display = '';
  const tituloEl = document.getElementById('modificar-titulo-examen');
  tituloEl.textContent = '✏️ ' + seccionId.toUpperCase().replace('IAR', 'IAR ').replace(/(\D)(\d)/, '$1 $2');
  const loadingEl = document.getElementById('modificar-loading');
  const listaEl = document.getElementById('modificar-lista-preguntas');
  loadingEl.style.display = '';
  listaEl.innerHTML = '';
  window.scrollTo(0, 0);
  try {
    const snap = await getDoc(doc(db, 'preguntas', seccionId));
    if (!snap.exists()) { loadingEl.textContent = '⚠️ No se encontró este examen en la base de datos.'; return; }
    const preguntas = snap.data().preguntas || [];
    _preguntasEditor = JSON.parse(JSON.stringify(preguntas));
    loadingEl.style.display = 'none';
    _renderizarEditor(preguntas);
  } catch(err) {
    loadingEl.textContent = '⚠️ Error al cargar: ' + (err.message || err);
    console.error('[IAR Mod14] Error cargando examen:', err);
  }
};

function _renderizarEditor(preguntas) {
  const listaEl = document.getElementById('modificar-lista-preguntas');
  listaEl.innerHTML = '';
  preguntas.forEach((pregData, qIdx) => {
    const card = document.createElement('div');
    card.className = 'mod-pregunta-card';
    card.id = 'mod-card-' + qIdx;
    const correctaOriginal = Array.isArray(pregData.correcta) ? pregData.correcta : [];
    card.innerHTML = `
      <div class="mod-pregunta-num">Pregunta ${qIdx + 1}</div>
      <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:14px;">
        <div class="mod-enunciado-texto" id="mod-enunciado-texto-${qIdx}" style="flex:1;">${_escHtml(pregData.pregunta || '')}</div>
        <button class="mod-btn-reescribir" onclick="window._modToggleEnunciado(${qIdx})">REESCRIBIR</button>
      </div>
      <textarea class="mod-textarea" id="mod-enunciado-ta-${qIdx}" style="display:none;" oninput="window._modMarcarCambio(${qIdx})">${_escHtml(pregData.pregunta || '')}</textarea>
      <div id="mod-opciones-${qIdx}">
        ${(pregData.opciones || []).map((opc, oi) => {
          const esCorrecta = correctaOriginal.includes(oi);
          return `
          <div class="mod-opcion-row" id="mod-opcion-row-${qIdx}-${oi}">
            <span class="mod-opcion-letra">${String.fromCharCode(65 + oi)}.</span>
            <span class="mod-opcion-texto" id="mod-opc-txt-${qIdx}-${oi}">${_escHtml(opc)}</span>
            <input class="mod-opcion-input" id="mod-opc-inp-${qIdx}-${oi}" style="display:none;" value="${_escHtml(opc)}" oninput="window._modMarcarCambio(${qIdx})" />
            <select class="mod-select-correcta ${esCorrecta ? 'correcta' : 'incorrecta'}" id="mod-sel-${qIdx}-${oi}" onchange="window._modCambiarCorrectez(${qIdx},${oi},this)">
              <option value="correcta" ${esCorrecta ? 'selected' : ''}>CORRECTA</option>
              <option value="incorrecta" ${!esCorrecta ? 'selected' : ''}>INCORRECTA</option>
            </select>
            <button class="mod-btn-reescribir" onclick="window._modToggleOpcion(${qIdx},${oi})">REESCRIBIR</button>
          </div>`;
        }).join('')}
      </div>
      <div style="display:flex;align-items:center;flex-wrap:wrap;gap:4px;">
        <button class="mod-btn-guardar" onclick="window._modGuardar(${qIdx})">💾 GUARDAR</button>
        <button class="mod-btn-descartar" onclick="window._modDescartar(${qIdx})" style="display:none;" id="mod-btn-descartar-${qIdx}">✕ Descartar cambios</button>
        <span class="mod-audit-log" id="mod-audit-${qIdx}"></span>
      </div>
    `;
...
window._modToggleEnunciado = function(qIdx) {
  const textoEl = document.getElementById('mod-enunciado-texto-' + qIdx);
  const taEl = document.getElementById('mod-enunciado-ta-' + qIdx);
  if (taEl.style.display === 'none') {
    taEl.value = _preguntasEditor[qIdx].pregunta || '';
    taEl.style.display = ''; textoEl.style.display = 'none'; taEl.focus();
  } else { textoEl.style.display = ''; taEl.style.display = 'none'; }
};

window._modToggleOpcion = function(qIdx, oi) {
  const txtEl = document.getElementById('mod-opc-txt-' + qIdx + '-' + oi);
  const inpEl = document.getElementById('mod-opc-inp-' + qIdx + '-' + oi);
  if (inpEl.style.display === 'none') {
    inpEl.value = _preguntasEditor[qIdx].opciones[oi] || '';
    inpEl.style.display = ''; txtEl.style.display = 'none'; inpEl.focus();
  } else { txtEl.style.display = ''; inpEl.style.display = 'none'; }
};

window._modMarcarCambio = function(qIdx) {
  const card = document.getElementById('mod-card-' + qIdx);
  if (card) card.classList.add('tiene-cambios');
  const btnDesc = document.getElementById('mod-btn-descartar-' + qIdx);
  if (btnDesc) btnDesc.style.display = '';
  _cambiosPendientes[qIdx] = true;
};

window._modCambiarCorrectez = function(qIdx, oi, sel) {
  sel.className = 'mod-select-correcta ' + sel.value;
  window._modMarcarCambio(qIdx);
};

window._modDescartar = function(qIdx) {
  const pregOriginal = _preguntasEditor[qIdx];
  const taEl = document.getElementById('mod-enunciado-ta-' + qIdx);
  const textoEl = document.getElementById('mod-enunciado-texto-' + qIdx);
  if (taEl) { taEl.value = pregOriginal.pregunta || ''; taEl.style.display = 'none'; }
  if (textoEl) { textoEl.innerHTML = _escHtml(pregOriginal.pregunta || ''); textoEl.style.display = ''; }
  const correctaOriginal = Array.isArray(pregOriginal.correcta) ? pregOriginal.correcta : [];
  (pregOriginal.opciones || []).forEach((opc, oi) => {
    const inpEl = document.getElementById('mod-opc-inp-' + qIdx + '-' + oi);
    const txtEl = document.getElementById('mod-opc-txt-' + qIdx + '-' + oi);
    const selEl = document.getElementById('mod-sel-' + qIdx + '-' + oi);
    if (inpEl) { inpEl.value = opc; inpEl.style.display = 'none'; }
    if (txtEl) { txtEl.innerHTML = _escHtml(opc); txtEl.style.display = ''; }
    if (selEl) { const esCorr = correctaOriginal.includes(oi); selEl.value = esCorr ? 'correcta' : 'incorrecta'; selEl.className = 'mod-select-correcta ' + (esCorr ? 'correcta' : 'incorrecta'); }
  });
  const card = document.getElementById('mod-card-' + qIdx);
  if (card) card.classList.remove('tiene-cambios');
  const btnDesc = document.getElementById('mod-btn-descartar-' + qIdx);
  if (btnDesc) btnDesc.style.display = 'none';
  delete _cambiosPendientes[qIdx];
};

function _leerEstadoEditor(qIdx) {
  const pregOriginal = _preguntasEditor[qIdx];
  const taEl = document.getElementById('mod-enunciado-ta-' + qIdx);
  const nuevaPregunta = taEl ? taEl.value : pregOriginal.pregunta;
  const nuevasOpciones = (pregOriginal.opciones || []).map((opc, oi) => {
    const inpEl = document.getElementById('mod-opc-inp-' + qIdx + '-' + oi);
    return inpEl ? inpEl.value : opc;
  });
  const nuevosIndicesCorrectos = (pregOriginal.opciones || []).map((_, oi) => {
    const selEl = document.getElementById('mod-sel-' + qIdx + '-' + oi);
    return selEl && selEl.value === 'correcta' ? oi : -1;
  }).filter(i => i !== -1);
  return { nuevaPregunta, nuevasOpciones, nuevosIndicesCorrectos };
}

window._modGuardar = function(qIdx) {
  const { nuevaPregunta, nuevasOpciones, nuevosIndicesCorrectos } = _leerEstadoEditor(qIdx);
  _mostrarConfirmacionGuardar(qIdx, async () => {
    const btnGuardar = document.querySelector('#mod-card-' + qIdx + ' .mod-btn-guardar');
    if (btnGuardar) { btnGuardar.disabled = true; btnGuardar.textContent = 'Guardando...'; }
    try {
      const snap = await getDoc(doc(db, 'preguntas', _seccionEditorActual));
      if (!snap.exists()) throw new Error('Documento no encontrado en Firestore');
      const data = snap.data();
      const preguntas = data.preguntas || [];
      const pregActualizada = { ...preguntas[qIdx], pregunta: nuevaPregunta, opciones: nuevasOpciones, correcta: nuevosIndicesCorrectos };
      preguntas[qIdx] = pregActualizada;
      const usuario = auth.currentUser ? auth.currentUser.email : 'admin';
      const timestamp = new Date().toLocaleString('es-AR');
      const logEntry = `Modificado por ${usuario} el ${timestamp}`;
      if (!Array.isArray(preguntas[qIdx].auditoria)) preguntas[qIdx].auditoria = [];
      preguntas[qIdx].auditoria.push(logEntry);
      await setDoc(doc(db, 'preguntas', _seccionEditorActual), { ...data, preguntas });
      _preguntasEditor[qIdx] = { ...pregActualizada };
      if (window.preguntasPorSeccion && window.preguntasPorSeccion[_seccionEditorActual]) {
        window.preguntasPorSeccion[_seccionEditorActual][qIdx] = { ...pregActualizada };
      }
      const card = document.getElementById('mod-card-' + qIdx);
      if (card) card.classList.remove('tiene-cambios');
      const btnDesc = document.getElementById('mod-btn-descartar-' + qIdx);
      ...
      delete _cambiosPendientes[qIdx];
      const textoEl = document.getElementById('mod-enunciado-texto-' + qIdx);
      if (textoEl) { textoEl.innerHTML = _escHtml(nuevaPregunta); textoEl.style.display = ''; }
      const taEl = document.getElementById('mod-enunciado-ta-' + qIdx);
      if (taEl) { taEl.value = nuevaPregunta; taEl.style.display = 'none'; }
      nuevasOpciones.forEach((opc, oi) => {
        const txtEl = document.getElementById('mod-opc-txt-' + qIdx + '-' + oi);
        if (txtEl) { txtEl.innerHTML = _escHtml(opc); txtEl.style.display = ''; }
        const inpEl = document.getElementById('mod-opc-inp-' + qIdx + '-' + oi);
        if (inpEl) { inpEl.value = opc; inpEl.style.display = 'none'; }
      });
      const auditEl = document.getElementById('mod-audit-' + qIdx);
      if (auditEl) auditEl.textContent = '✅ Guardado — ' + logEntry;
      if (btnGuardar) { btnGuardar.disabled = false; btnGuardar.textContent = '💾 GUARDAR'; }
    } catch(err) {
      console.error('[IAR Mod14] Error guardando:', err);
      alert('⚠️ Error al guardar: ' + (err.message || err));
      if (btnGuardar) { btnGuardar.disabled = false; btnGuardar.textContent = '💾 GUARDAR'; }
    }
  });
};

function _mostrarConfirmacionGuardar(qIdx, onAceptar) {
  const overlay = document.createElement('div');
  overlay.id = 'mod-confirm-overlay';
  overlay.innerHTML = `
    <div class="mod-confirm-box">
      <div style="font-size:2rem;margin-bottom:10px;">⚠️</div>
      <div style="font-size:1.1rem;font-weight:800;color:#d97706;margin-bottom:10px;">¿ESTÁS SEGURO DE GUARDAR LOS CAMBIOS?</div>
      <div style="font-size:.88rem;color:#475569;line-height:1.55;margin-bottom:20px;">
        Los cambios afectarán a <strong>toda la plataforma</strong> de forma permanente.
      </div>
      <div style="display:flex;gap:10px;">
        <button id="mod-confirm-cancelar" style="flex:1;padding:11px;background:#f1f5f9;color:#475569;border:1.5px solid #cbd5e1;border-radius:8px;font-size:.92rem;font-weight:700;cursor:pointer;">CANCELAR</button>
        <button id="mod-confirm-aceptar" style="flex:1;padding:11px;background:linear-gradient(135deg,#dc2626,#b91c1c);color:#fff;border:none;border-radius:8px;font-size:.92rem;font-weight:700;cursor:pointer;">ACEPTAR</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);
  document.getElementById('mod-confirm-cancelar').addEventListener('click', () => overlay.remove());
